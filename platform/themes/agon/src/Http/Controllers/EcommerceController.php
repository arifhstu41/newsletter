...
class EcommerceController extends PublicController
{
    ...
    public function ajaxGetProductReviews(string|int $id, Request $request): BaseHttpResponse
    {
        ...
        if ($star) {
            $message = __(':total review(s) ":star :unit_star" for ":product"', [
                'total' => $reviews->total(),
                'product' => $product->name,
                'star' => $star,
                'unit_start' => $star > 1 ? 'stars' : 'star',
            ]);
        } else {
        ...
        return $this->httpResponse
            ->setData(
                Theme::partial('ecommerce.review-list', compact('reviews'))
            )
            ->setMessage($message);
    }

    public function ajaxGetProductReviewDetails(string|int $id, Request $request)
    {
        $product = Product::query()
            ->where('status', BaseStatusEnum::PUBLISHED)
            ->where('is_variation', false)
